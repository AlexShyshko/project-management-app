...
export class ApiService {
  ...
  private handleError(error: HttpErrorResponse) {
    if (error.status === 0) {
      ...
      this.snackBar.open(`Backend returned code ${error.status}, message was: ${error.error.message}`, 'Error!', {
  ...
  public updateCertainTask(token: string, oldTask: Task, newTask: Task): Observable<Task> {
